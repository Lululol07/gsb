...
/** 
 * Vérifie si une chaîne fournie est bien un mois avec une année valide, au format AAAA-MM.                     
 * 
 * Retrourne true si la chaîne $date est un mois avec une année valide, au format AAAA-MM, false sinon.
 * @param string date à vérifier
 * @return boolean succès ou échec
 */ 
function estMoisValide($date) {
    // vérification du format de la date via une expression regulière
    if(preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])$/", $date) == 0) return false;

    // Casse la chaîne $date pour extraire les 4 premiers caractères (l'année) et les 2 derniers (le mois)
    $tabDate = explode('-', $date);

    // Vérifie si le premier jour du mois et de l'année donnée est valide
    if(!checkdate($tabDate[1], 1, $tabDate[0])) return false;

    // Si aucune des conditions ci-dessus est vrais, alors la date passer en paramètre est valide
    return true;
}
