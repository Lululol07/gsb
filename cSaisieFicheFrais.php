<?php
    ...
    if ( !$existeFicheFrais ) {
        // Fermeture de la fiche de frais du mois précédent
        $ancienneDate = new DateTime(date("Y-m-01"));
        $ancienneDate->modify("-1 month");
        modifierEtatFicheFrais($idConnexion, $ancienneDate->format("Ym"), obtenirIdUserConnecte(), 'CL');
        // Création de la fiche de frais
        ajouterFicheFrais($idConnexion, $mois, obtenirIdUserConnecte());
    }
    // Vérifie si la fiche est cloturée ou non
    $ficheCloturee = obtenirEtatFicheFrais($idConnexion, $mois, obtenirIdUserConnecte()) != 'CR';
    ...
    // Ajoute une erreur et empeche la modification si la fiche n'est pas en cours de saisie
    if ($ficheCloturee) {
        ajouterErreur($tabErreurs, "La fiche est n'est plus en cours de saisie et par conséquent ne peut plus être modifiée.");
    } else {
